...
if isempty(rawCell) || isempty(numData)
    error('File could not be read or is empty: %s', inputFile);
end

if size(numData,2) < 15
    error('File does not contain enough columns.');
end

Timestamp = rawCell(:,1);

if ischar(Timestamp{1}) || isstring(Timestamp{1})
    firstEntry = string(Timestamp{1});
    if strcmpi(strtrim(firstEntry), "Timestamp")
...
% Columns
Amplitude = numData(:,4);    % PSL_UPMU_Voltage0_Magnitude_L1MagAng
Phase     = numData(:,3);    % PSL_UPMU_Voltage0_Angle_L1MagAng 
Frequency = numData(:,15);   % PSL_UPMU_Frequency

nRows = min([numel(Timestamp), size(numData,1), numel(Amplitude), numel(Frequency), numel(Phase)]);
